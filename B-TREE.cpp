...
bond* search(btree* &x,int k){                                     //for finding the node containg key
	...
	if(i<x->n && k==x->arr[i].key){
	   ...
	   return temp;
		...
		cout<<"\nSorry,"<<k<<" is not in the B-tree(search)\n";
		return NULL;
	}
	else
...
bond* temp_search(btree* &x,int k,bond* &temp){                    //for finding the parent node of a node containg key
	...
	if(i<x->n && k==x->arr[i].key)
		...
		cout<<"\nSorry,"<<k<<" is not in the B-tree(temp_search)\n";
		return NULL;
	}
	else{
		temp2->spy=x;
...
//       insert(root,i);
	...
	cout<<"\n";
//	delete_key(root,4);
//	finder=search(root,4);
	delete_key(root,8);
	finder=search(root,8);
//	delete_key(root,13);
//	finder=search(root,13);

    cout<<"I am root "<<root->arr[0].key<<"\n";
	traverse(root);
}
